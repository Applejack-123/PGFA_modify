...
class Processor:
    ...
    def save_accuracy_figure(
        ...
        save_path = os.path.join(output_dir, f"split_{split}_fusion_{prefix}_accuracy.png")
    ...
    def test_epoch(
        ...
        support_factor,
        split,
        sample_result_dir="./output/sample_results",
    ):
        self.encoder.eval()
        self.proj.eval()
        self.fusion.eval()
        ...
        with torch.no_grad():
            for data, label, rgb in tqdm(loader, desc=f"Test epoch {epoch}"):
                ...
                rgb_feat = rgb.float().cuda(non_blocking=True)
